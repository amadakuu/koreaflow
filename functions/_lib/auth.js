...
async function sign(value, secret) {
  ...
  return base64url(new Uint8Array(await crypto.subtle.sign('HMAC', key, encodeText(value))));
}
...
export async function createSessionCookie(user, secret) {
  const payload = base64url(encodeText(JSON.stringify({
    id: user.id,
    email: user.email,
    exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 30,
  })));
  return cookie(SESSION_COOKIE, `${payload}.${await sign(payload, secret)}`, { maxAge: 60 * 60 * 24 * 30 });
}
...
export function oauthStateCookie(state) {
  return cookie(STATE_COOKIE, state, { maxAge: 600 });
}
